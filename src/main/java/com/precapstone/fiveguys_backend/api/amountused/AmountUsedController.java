...
public class AmountUsedController {
    ...
    @GetMapping("week/{localDate}")
    public ResponseEntity readByFewDay(@RequestHeader("Authorization") String authorization, @PathVariable LocalDate localDate) {
        var accessToken = authorization.replace(JwtFilter.TOKEN_PREFIX, "");

        var dailyAmounts = amountUsedService.readByFewDay(accessToken, localDate);
        var response = CommonResponse.builder().code(200).message("주간 사용량 조회 성공").data(dailyAmounts).build();
        return ResponseEntity.ok(response);
    }
}
